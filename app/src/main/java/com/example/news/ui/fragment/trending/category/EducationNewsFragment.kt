...
@AndroidEntryPoint
class EducationNewsFragment : BaseFragment<FragmentTrendingPagesBinding>(
    FragmentTrendingPagesBinding::inflate
) {

    private val mAdapter by lazy { ItemTrendingNewsAdapter("Giáo Dục") }
    ...
    override fun obServerLivedata() {
        mainViewModel.checkInternet.observe(viewLifecycleOwner){ checkInternet ->
            if(!checkInternet){
                if(mainViewModel.checkEducation == null ){
                    binding.errorImageView.visibility = View.VISIBLE
                    binding.errorTextView.visibility = View.VISIBLE
                }else{
                    mainViewModel.checkEducation = false
                }
            }else{
                if(mainViewModel.checkEducation == null){
                    mainViewModel.checkEducation = true
        ...
        mainViewModel.newsEducationResponse.observe(viewLifecycleOwner) { networkResult ->
            when (networkResult) {
                ...
                is NetworkResult.Error -> {
                    binding.progressBar.visibility = View.GONE
                    if (mainViewModel.checkEducation == false) {
                        networkResult.data?.let { mAdapter.setData(it) }
                    }
                }
    ...
    private fun requestApiData(){
        mainViewModel.getEducationNews(Constants.Q_EDUCATION)
    }
