...
@AndroidEntryPoint
class IdeaNewsFragment : BaseFragment<FragmentTrendingPagesBinding>(
    FragmentTrendingPagesBinding::inflate
) {

    private val mAdapter by lazy { ItemTrendingNewsAdapter("Ý Kiến") }
    ...
    override fun obServerLivedata() {
        mainViewModel.checkInternet.observe(viewLifecycleOwner){ checkInternet ->
            if(!checkInternet){
                if(mainViewModel.checkIdea == null){
                    binding.errorImageView.visibility = View.VISIBLE
                    binding.errorTextView.visibility = View.VISIBLE
                }else{
                    mainViewModel.checkIdea = false
                }
            }else{
                if(mainViewModel.checkIdea == null){
                    mainViewModel.checkIdea = true
        ...
        mainViewModel.newsIdeaResponse.observe(viewLifecycleOwner) { networkResult ->
            when (networkResult) {
                ...
                is NetworkResult.Error -> {
                    binding.progressBar.visibility = View.GONE
                    if (mainViewModel.checkIdea == false) {
                        networkResult.data?.let { mAdapter.setData(it) }
                    }
                }
    ...
    private fun requestApiData(){
        mainViewModel.getIdeaNews(Constants.Q_IDEA)
    }
